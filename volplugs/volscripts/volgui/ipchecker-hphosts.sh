...
if [ ! -d "$HOME/$CASE/$DDIR/ipfiles/hphosts" ]; then
        mkdir -p $HOME/$CASE/$DDIR/ipfiles/hphosts
fi
...
cd $HOME/$CASE/$DDIR
echo "We are going to grab information from the hpHosts database for each our IPs."
wc -l ipfiles/ipv4connections.external.txt
echo " "
echo "I am going to wait a little bit between each request, so I do not "
echo "request to many too quickly, and anger the hpHosts folks. "
echo " ";
while read i; do
	# hpHosts 
        echo " Checking the hpHosts database..... $i"
	wget http://hosts-file.net/?s=$i -O ipfiles/hpHosts/$i.hpHosts.html
        sleep 5;
	echo " "
