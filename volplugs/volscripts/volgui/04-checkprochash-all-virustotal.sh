...
if [ $INSPECT == "1" ]; then
	...
	if [ ! -f "procexedump.hashes.txt" ]; then
	...
	python $VTSE -k $APIK -f procexedump.hashes.txt
elif [ $INSPECT == "2" ]; then
        cd $HOME/$CASE/procmemdump
        if [ ! -f "procmemdump.hashes.txt" ]; then
                echo "It does not look as if the master hash list is in this directory."
                echo "Please dump the executables, and hash them before running this script."
                echo " "
		cd $HOME;
	...
	python $VTSE -k $APIK -f $HOME/$CASE/procmemdump/procmemdump.hashes.txt
else
