...
if [ ! -d "$HOME/$CASE/$DDIR/ipfiles/cleanmx" ]; then
        mkdir -p $HOME/$CASE/$DDIR/ipfiles/cleanmx
fi
...
cd $HOME/$CASE/$DDIR
echo "We are going to grab information from the cleanmx database for each our IPs."
wc -l ipfiles/ipv4connections.external.txt
echo " "
echo "I am going to wait a little bit between each request, so I do not "
echo "request to many too quickly, and anger the cleanmx folks. "
echo " ";
while read i; do
        # cleanmx
        echo " Checking the BFK Database..... $i"
        wget http://support.clean-mx.de/clean-mx/viruses.php?sort=firstseen%20desc&review=$i -O ipfiles/cleanmx/$i.cleanmx.phish.html
        sleep 5;
        echo " "
