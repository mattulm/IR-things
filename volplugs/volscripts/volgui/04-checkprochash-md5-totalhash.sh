...
################################################################
if [ $INSPECT == "1" ]; then
        if [ ! -f "$HOME/$CASE/procexedump/procexedump.md5.stripped.txt" ]; then
	...
	while read -r line; do
	        wget http://totalhash.com/search/hash:$line -O totalhash/$line.md5.thash.html
	        sleep 1
	done < procexedump.md5.stripped.txt
elif [ $INSPECT == "2" ]; then
	...
	mkdir p totalhash
        while read -r line; do
                wget http://totalhash.com/search/hash:$line -O totalhash/$line.md5.thash.html
                sleep 1
        done < procmemdump.md5.stripped.txt
else
